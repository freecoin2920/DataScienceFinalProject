...
Which country's exchange rate was impacted the most tdue to the us great recessionthrough 2007-2009, which country was least impacted? how did unemployment look in both countries? was there a relationship?

```{r Ex-1}
exchange_greatRec_high <- ExchangeRate%>% group_by(LOCATION) %>% filter(TIME==2009)%>% summarize(max=max(Value)) %>% arrange(desc(max)) %>%
slice(1, n()) 
...
trainRowNumbers <- createDataPartition(CER$'LOCATION', p=0.8, list=FALSE)
