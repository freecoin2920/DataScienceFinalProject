...
CHN_exp <- NetExports[c(1,3,4,6,7)] %>% filter(TIME>=1995, Country == "CHN", SUBJECT=="EXP") %>% rename(exports = Value) %>% rename(Year = TIME)

CHN_netTrade <- NetExports[c(1,3,4,6,7)] %>% filter(TIME>=1995, Country == "CHN", SUBJECT=="NTRADE") %>% rename(net_Trade = Value) %>% rename(Year = TIME)

CHN_unemployment <-unemploymentRate[c(1,3,4)] %>% filter( Year>=1995) %>% filter(Country == 'China')

CHN_compare <- inner_join(CHN_unemployment,CHN_exp, by = "Year")

CHN_compare2 <- inner_join(CHN_unemployment,CHN_netTrade, by = "Year")
...
ggplot(CHN_compare,
  ...
  labs(x = "Exports", y = "Year",
       size = "Unemployment",
       title = "Relationship between Unemployment and exports of Chnia")
```
...
ggplot(CHN_compare2,
  ...
  labs(x = "Net trade", y = "Year",
       size = "Unemployment",
       title = "Relationship between Unemployment and Net Trade of China")
```
